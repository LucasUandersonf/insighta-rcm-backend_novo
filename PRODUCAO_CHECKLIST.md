...
- [x] **Catálogo de eventos ampliado — IMPLEMENTADO EM RODADA POSTERIOR**: `denial_appeal.resolved` (DenialAppealService.resolve_appeal, nas três transições) e `no_show_risk.high` (AppointmentService.create_appointment, só nível "alto"). Ambos deliberadamente NÃO ligados ao path de ingestão em lote da Agenda (`normalization_service.py`) — um arquivo com centenas de linhas geraria dezenas de chamadas HTTP síncronas durante o upload.
...
- [x] **Snapshot mensal da Nota de Saúde — IMPLEMENTADO NESTA RODADA**: `app/worker/health_score_snapshot_job.py` (Sala de Comando 2.0) tinha o mesmo problema de agendamento dos 3 jobs acima — resolvido com um serviço de cron NATIVO do Railway (não uma variável de ambiente nova, não um serviço externo tipo EventBridge), rodando `python -m app.scripts.run_worker app.worker.health_score_snapshot_job` 1x por mês. `app/scripts/run_worker.py` é novo e GENÉRICO — resolve o mesmo bootstrap (DATABASE_URL de app_runtime, ver DECISÃO em entrypoint.py) para qualquer worker, então os 3 jobs acima (`weekly_report_job.py`/`daily_alert_job.py`/`platform_risk_alert_job.py`) também podem usar o mesmo mecanismo, só faltando criar o serviço de cron de cada um no Railway quando fizer sentido.
